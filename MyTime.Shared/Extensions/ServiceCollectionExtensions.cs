            ...
            services.AddDbContext<MyTimeDbContext>(options =>
            {
                ...
                if (isDevelopment)
                {
                    options.UseSeeding((context, _) =>
                    {
                        ...
                        Punch punch1 = new() { PunchedUserId = 1, PunchedTimeUtc = DateTime.UtcNow.AddHours(-5), PunchType = Enums.PunchType.In };
                        Punch punch2 = new() { PunchedUserId = 1, PunchedTimeUtc = DateTime.UtcNow.AddHours(-1), PunchType = Enums.PunchType.Out };
